...
class ParameterManager:
    ...
    def collect_module_parameters_for_template(self, base_namespace: str = "") -> List[Dict[str, Any]]:
        ...
        """
        module_data = []
        self._collect_module_parameters_recursive(self.instance, module_data, base_namespace)
        return module_data
    
    def _collect_module_parameters_recursive(self, instance: 'Instance', 
                                           module_data: List[Dict[str, Any]], 
                                           current_namespace: str = "") -> None:
        """Recursively collect module parameters with their full namespaces.
        ...
        if instance.element_type == "module":
            ...
            parameter_paths, configurations = self._extract_module_parameter_info(instance)
            
            if parameter_paths or configurations:
                module_info = {
                    "node": full_namespace,
        ...
        if hasattr(instance, 'children') and instance.children:
            for child in instance.children.values():
                self._collect_module_parameters_recursive(child, module_data, current_namespace)
    ...
    def _extract_module_parameter_info(self, module_instance: 'Instance') -> tuple[Dict[str, str], List[Dict[str, Any]]]:
        ...
        for param in module_instance.parameter_manager.get_all_parameters():
        ...
        return parameter_paths, configurations
    ...
    def generate_parameter_set_template(self, deployment_name: str, 
        ...
        module_data = self.collect_module_parameters_for_template()
    ...
    def _create_namespace_structure_and_copy_configs(self, module_data: Dict[str, Any], 
        ...
        Args:
            module_data: Module data containing node path and parameters
            parameter_set_root: Root directory for parameter set
        """
        node_path = module_data["node"]
        parameter_paths = module_data["parameter_paths"]
        ...
        updated_parameter_paths = {}
        for param_name, original_path in parameter_paths.items():
            ...
            updated_parameter_paths[param_name] = relative_path
        
        # Update module data with new paths
        module_data["parameter_paths"] = updated_parameter_paths
