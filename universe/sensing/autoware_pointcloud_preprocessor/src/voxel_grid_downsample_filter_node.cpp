...
class VoxelGridDownsampleFilterComponent : public rclcpp::Node
{
public:
  explicit VoxelGridDownsampleFilterComponent(const rclcpp::NodeOptions & options)
  : Node("VoxelGridDownsampleFilterComponent", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&VoxelGridDownsampleFilterComponent::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "VoxelGridDownsampleFilterComponent started");
  }
