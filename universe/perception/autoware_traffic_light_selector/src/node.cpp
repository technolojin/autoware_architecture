...
{
public:
  explicit TrafficLightSelectorNode(const rclcpp::NodeOptions & options)
  : Node("traffic_light_selector", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&TrafficLightSelectorNode::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "TrafficLightSelectorNode started");
  }
