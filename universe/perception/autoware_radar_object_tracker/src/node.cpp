...
{
public:
  explicit RadarObjectTrackerNode(const rclcpp::NodeOptions & options)
  : Node("radar_object_tracker", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&RadarObjectTrackerNode::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "RadarObjectTrackerNode started");
  }
