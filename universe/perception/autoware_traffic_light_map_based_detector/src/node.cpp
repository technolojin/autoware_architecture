...
{
public:
  explicit MapBasedDetector(const rclcpp::NodeOptions & options)
  : Node("traffic_light_map_based_detector", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&MapBasedDetector::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "MapBasedDetector started");
  }
