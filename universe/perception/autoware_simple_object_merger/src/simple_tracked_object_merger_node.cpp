...
{
class SimpleTrackedObjectMergerNode : public rclcpp::Node
{
public:
  explicit SimpleTrackedObjectMergerNode(const rclcpp::NodeOptions & options)
  : Node("simple_tracked_object_merger_node", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&SimpleTrackedObjectMergerNode::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "SimpleTrackedObjectMergerNode started");
  }
