...
{
class SimpleDetectedObjectMergerNode : public rclcpp::Node
{
public:
  explicit SimpleDetectedObjectMergerNode(const rclcpp::NodeOptions & options)
  : Node("simple_object_merger", options)
  {
    // Create a timer that calls the callback every 2 seconds (1/2 Hz)
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2),
      std::bind(&SimpleDetectedObjectMergerNode::timer_callback, this));
    
    RCLCPP_INFO(this->get_logger(), "SimpleDetectedObjectMergerNode started");
  }
